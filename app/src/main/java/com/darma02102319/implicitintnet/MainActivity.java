package com.darma02102319.implicitintnet;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void tampilKalender(View view){
        Intent kalenderInten = new Intent(Intent.ACTION_MAIN);
        kalenderInten.addCategory(Intent.CATEGORY_APP_CALENDAR);
        startActivity(kalenderInten);
    }
    ...
    public void  tampilKalkulator(View view)
    {
        ...
        for (PackageInfo pi : packs)
        {
            ...
            String packageName_lowCase = packageName.toLowerCase();

            if (packageName_lowCase.contains("calcul"))
            {
        ...
        if (item_size >=1 )
        {
            String pacageName = (String) items.get(0).get("pacakageName");

            Intent i = pm.getLaunchIntentForPackage(pacageName);
